...
$(function() {
    console.log("hi"); 
    $('#fitnessSubmitPost').click(handleSubmit);
    $('#fitnessCancelPost').click(resetPostBox);
    renderFitness(); 
    //deleteTest(); 
});
...
const handleSubmit = async function(event) {
    ...
    if ($.trim($(`#postContent`).val()) == "" || $.trim($(`#postTitle`).val()) == "") {
        ...
        $(`#fitnessPostForm`).append(`<p id = "fillAllFieldsWarning" style = "color: red"> **Must fill out all fields!** </p>`); 
        }
    } else {
  ...
  "July", "August", "September", "October", "November", "December"
    ...
    let result = axios.post('http://localhost:3000/private/fitness/' + id, {
        data: {
            id: id,
            author: username,
            body: content,
...
function resetPostBox() {
    $(`#fitnessPostForm`).replaceWith(`<div class="form-group" id = "fitnessPostForm">
...
async function renderFitness() {
    ...
    const result = await axios({
        method: 'get',
        url: 'http://localhost:3000/private/fitness',
        headers: {
            Authorization: "Bearer " + jwt
        },
      });
    objectArray = Object.values(result.data.result)
    let posts = `<div id = "fitnesspostcontainer">`;
    ...
    posts += `</div>`;
    $('#fitnesspostcontainer').replaceWith(posts);
...
const handleDelete = async function(event) {
    ...
    const result = await axios({
        method: 'delete',
        url: 'http://localhost:3000/private/fitness/' + id,
        headers: {
            Authorization: "Bearer " + jwt
        },
      });
...
const handleUpdate = async function(event) {
    ...
    console.log(result)
    let result2 = axios.post('http://localhost:3000/private/fitness/' + id, {
        data: {

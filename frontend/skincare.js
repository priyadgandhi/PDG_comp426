...
$(function() {
    console.log("hi"); 
    $('#skincareSubmitPost').click(handleSubmit);
    $('#skincareCancelPost').click(resetPostBox);
    renderSkincare(); 
    //deleteTest(); 
});
...
const handleSubmit = async function(event) {
    ...
    if ($.trim($(`#postContent`).val()) == "" || $.trim($(`#postTitle`).val()) == "") {
        ...
        $(`#skincarePostForm`).append(`<p id = "fillAllFieldsWarning" style = "color: red"> **Must fill out all fields!** </p>`); 
        }
    } else {
  ...
  "July", "August", "September", "October", "November", "December"
    ...
    let result = axios.post('http://localhost:3000/private/skincare/' + id, {
        data: {
            id: id,
            author: username,
            body: content,
...
function resetPostBox() {
    $(`#skincarePostForm`).replaceWith(`<div class="form-group" id = "skincarePostForm">
...
async function renderSkincare() {
    ...
    const result = await axios({
        method: 'get',
        url: 'http://localhost:3000/private/skincare',
        headers: {
            Authorization: "Bearer " + jwt
        },
      });
    objectArray = Object.values(result.data.result)
    let posts = `<div id = "skincarepostcontainer">`;
    ...
    posts += `</div>`;
    $('#skincarepostcontainer').replaceWith(posts);
...
const handleDelete = async function(event) {
    ...
    const result = await axios({
        method: 'delete',
        url: 'http://localhost:3000/private/skincare/' + id,
        headers: {
            Authorization: "Bearer " + jwt
        },
      });
...
const handleUpdate = async function(event) {
    ...
    console.log(result)
    let result2 = axios.post('http://localhost:3000/private/skincare/' + id, {
        data: {
